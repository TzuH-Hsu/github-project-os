...
3. Pick the type deliberately — release-please reads it directly into the changelog:
   ...
   ask whether the type is right: a change users will notice is usually a `feat`
   or a `fix`, not a hidden type. When the type is genuinely right and a tag is
   still needed (a new CI gate under `ci` that adopters must pick up), put
   `Release-As: X.Y.Z` in the PR body — the body becomes the squash commit's
   message — with X.Y.Z the current version plus one patch; anything larger means
   the type was wrong (see `skills/release-management` rule 2).
