...
- `Makefile` — where workflow logic lives (`lint`, `test`, `verify`, `ci-pr`, `ci-tools`), except the event handler in `scripts/issue-labeler.js` (rule 1, ADR-0008)
- `` `scripts/issue-labeler.js` `` / `` `scripts/check-issue-labeler.sh` `` — the one event handler and the `make check` step that tests it
