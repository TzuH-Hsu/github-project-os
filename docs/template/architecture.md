...
| `.github/workflows/issue-labeler.yml` | Form selections → labels (single-home preserving); `area:*` allowlist read from `labels.yml` at run time | `ALLOWED_PRIORITIES` / `ALLOWED_SUBTYPES` must match the forms — checked by `scripts/check-label-forms.sh` |
