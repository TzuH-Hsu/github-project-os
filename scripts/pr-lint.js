...
async function verifyIssue(github, owner, repo, number) {
  ...
  try {
    ...
    // read needs `issues: read` and `pull-requests: read` (ci.yml permissions).
    if (err && err.status === 403) throw new Error(`could not verify issue #${number}: ${err.message} — the workflow's GITHUB_TOKEN cannot read issues; on a private repository the job needs \`issues: read\` and \`pull-requests: read\` in ci.yml's permissions block`);
